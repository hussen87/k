...
def change_username(session_id, device_id, iid, last_username, new_username):
    ...
    if "unique_id" in result and check_is_changed(
        ...
        return "تم تغيير اليوزر بنجاح."
    else:
        return "لم يتم تغيير اليوزر والسبب : " + str(result)

#open('sub.txt','w').write('')
bot = telebot.TeleBot('6375923822:AAHA3ggNamRiDM3JaLF12hnfuKk0oy41sE4')
@bot.message_handler(commands=['t'])
def w(message):
    u = open("sub.txt").read()
    if str(message.from_user.id) in u:
        k = bot.reply_to(message,'- ارسل سشن')
        bot.register_next_step_handler(k,t, message.from_user.id)
    else:bot.reply_to(message,'- اسف لكنك لست مشترك')
def t(message, id):
    if str(message.from_user.id) == str(id) :
            ...
            if user != "None":
                l = bot.reply_to(message,'send users')
                bot.register_next_step_handler(l,llk, session_id)
            else:bot.reply_to(message,"سشنك عطلان")
def llk(message, session_id):
         ...
         user = get_profile(session_id, device_id, iid)
         if user != "None":
             l = bot.reply_to(message,f"- يوزرك الحالي : {user}")
             f = message.text
             bot.reply_to(message,change_username(session_id, device_id, iid, user, message.text))
             hg = bot.send_message(message.chat.id,'send user')
             bot.register_next_step_handler(hg,llk, session_id)
         else:
          bot.reply_to(message,"- سشنك عطلان")
@bot.message_handler(commands=['check'])
def w(message):
    u = open("sub.txt").read()
    if str(message.from_user.id) in u:
        k = bot.reply_to(message,'- ارسل سشن')
        bot.register_next_step_handler(k,ch, message.from_user.id)
    else:bot.reply_to(message,'- اسف لكنك لست مشترك')
def ch(message, id):
    if str(message.from_user.id) == str(id) :
     session_id = message.text
    ...
    else:bot.reply_bot(message,"سشنك عطلان")
@bot.message_handler(commands=['start'])
def khuks(message):
    u = open("sub.txt").read()
    if str(message.from_user.id) in u:
        k = bot.reply_to(message,'- ارسل سشن')
        bot.register_next_step_handler(k,qna, message.from_user.id)
    else:bot.reply_to(message,'- اسف لكنك لست مشترك')
...
def qna(message, id):
    ...
    if str(message.from_user.id) == str(id) :
     ...
     if user != "None":
         l = bot.reply_to(message,f"- يوزرك الحالي : {user}")
         f = bot.reply_to(l,'- الان ارسل اليوزر الذي تريده')
         bot.register_next_step_handler(f, nn, session_id, user, device_id, iid)
     else:
          bot.reply_to(message,"- سشنك عطلان")
    
def nn(message, session_id, user,device_id, iid):
    
    bot.reply_to(message,change_username(session_id, device_id, iid, user, message.text))
def lk(message, session_id, user,device_id, iid):
    
    
    lk(message, session_id, user,device_id, iid)
